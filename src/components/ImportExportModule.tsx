...
import {getManyItems, saveVideoRecord} from '../api/sqlLite';
import {IIcon} from '../utils/constant';

const color = Theme.DARK;
const font = Theme.FONTS;
interface ImpprtsModProps {
  modal: boolean;
  list: [];
...
const ImportExportModule: React.FC<ImpprtsModProps> = ({
  modal,
  setModal,
  list,
  setList,
    ...
    try {
      ...
      const promised = parsedFileContent.map(async item => {
        const record = {
          ...
          episodeIdGogo:
            item?.value?.episodeId || item?.episodeId || item?.episodeIdGogo,
      ...
      <View style={styles.container}>
        <ScrollView>
          ...
          <View
            ...
            <Text style={styles.ixText}>
              Create/Export your animeBackup so you can restore
            </Text>
            <TouchableOpacity
              style={[styles.ixBtn, {backgroundColor: color.AccentBlue}]}
